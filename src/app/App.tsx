  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* Верхняя панель */}
        <div style={{ pointerEvents: 'auto', maxWidth: '500px', width: '100%', margin: '0 auto' }}>
          <div className="bg-black/50 backdrop-blur-xl rounded-2xl px-4 py-3 border border-white/10 shadow-2xl">
            <div className="flex items-center justify-between mb-2">
              <motion.button onClick={returnToMenu}
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="flex items-center gap-4">
                  <motion.span key={`h${homeScore}`} initial={{ scale: 1.8 }} animate={{ scale: 1 }}
                    className="text-3xl font-black text-white tabular-nums">{homeScore}</motion.span>
                  <span className="text-white/30 text-xl">—</span>
                  <motion.span key={`a${awayScore}`} initial={{ scale: 1.8 }} animate={{ scale: 1 }}
                    className="text-3xl font-black text-white tabular-nums">{awayScore}</motion.span>
                </div>
            ...
            <div>
              ...
              <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
                <motion.div className="h-full rounded-full bg-gradient-to-r from-green-400 via-yellow-400 to-red-500"
                  animate={{ width: `${pressure}%` }} transition={{ duration: 0.5 }} />
              </div>
            </div>
          </div>
        </div>
